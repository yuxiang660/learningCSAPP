...
- [第四章 处理器体系结构](#第四章-处理器体系结构)
    ...
    - [Y86-64的流水线实现](#y86-64的流水线实现)
        ...
        - [预测下一个PC](#预测下一个pc)
        - [流水线冒险](#流水线冒险)
            - [用暂停来避免数据冒险](#用暂停来避免数据冒险)
            - [用转发来比避免数据冒险](#用转发来比避免数据冒险)
            - [加载/使用的数据冒险](#加载使用的数据冒险)
            - [避免控制冒险](#避免控制冒险)
        - [异常处理](#异常处理)
...
### 预测下一个PC
为了做到每个时钟周期都有一个条新指令进入执行阶段，必须在取出当前指令后，马上确定下一条指令的位置。但是，如果取出的指令时条件分支指令，要到几个周期否，才能知道是否要选择分支。类似地，如果取出的指令时ret，要到指令通过访存阶段，才能确定返回地址。

除了条件转移指令和ret指令，其他指令我们可以在取值阶段计算出下一条指令的地址。对于call和jmp(无条件转移)来说，下一条指令的地址时指令中的常数字valC，而对于其他指令来说就是valP。
* 对于条件转移指令，我们可以预测选择了分支，那么PC值应为valC，也可以预测没有选择分支，那么新PC值为valP。
* 对于ret指令，可能返回值位于栈顶的字，我们不会尝试任何预测，而是简单地暂停处理新指令。

上图中，“Predict PC”会从PC增加器计算出的valP和取指中得到的valC中进行选择。这个值存放在流水线寄存器F中，作为程序计数器的预测值。“Select PC”从三个值中选择一个作为指令内存的地址：
* 预测的PC
* 对于到达流水线寄存器M的不选择分支的指令来说时valP的值(存储在寄存器M_valA中)
* 或是当ret指令到达流水线寄存器W(存储在W_valM)时的返回地址的值

### 流水线冒险
流水线设计需要解决两个相关有可能产生的冒险问题：
* 数据相关
    * 下一条指令会用到这一条指令计算出的结果
* 控制相关
    * 一条指令要确定下一条指令的位置，例如在执行跳转、调用或返回指令时
![pipe_no_race](pictures/pipe_no_race.png)

![pipe_race](pictures/pipe_race.png)
* 上面的例子说明，如果一条指令的操作数被它前面三条指令中的任意一条改变的话，都会出现数据冒险。因为我们的流水线化的处理器是在译码阶段从寄存器文件中读取指令的操作数，而要到三个周期以后，指令经过写回阶段时，才会将指令的结果写到寄存器文件。

当一条指令更新后面指令会读到的程序状态时，冒险就有可能出现。程序状态的类型包括：
* 程序寄存器
    * 导致数据冒险
    * 因为寄存器文件的读写是在不同的阶段进行的
* 程序计数器
    * 导致了控制冒险
    * 取指阶段逻辑在取下一条指令之前，正确预测了程序计数器的新值时，就不会产生冒险
* 内存
    * 只有自我修改代码的程序会出现
* 条件码寄存器
    * 不会发生冒险
...
#### 用暂停来避免数据冒险
![pipe_race_halt](pictures/pipe_race_halt.png)

#### 用转发来比避免数据冒险
![pipe_race_bypass](pictures/pipe_race_bypass.png)
* 周期6的译码阶段发现，寄存器%rax是操作数valB的源寄存器，但在写端口E上还有一个对%rax的写未进行。因此，将提供到端口E的数据字(W_valE)作为操作数valB的值，就能避免暂停。

![pipe_bypass](pictures/pipe_bypass.png)
* 添加了五个不同的转发源：e_valE, m_valM, M_valE, W_valM, W_valE，以在不同情况下旁路不同的数据源，从而防止冒险的发生

#### 加载/使用的数据冒险
![pipe_load_race](pictures/pipe_load_race.png)
* 加载/使用的数据冒险不能单纯用转发来解决，因为内存读在流水线发生的比较晚
* 用暂停来处理加载/使用冒险的方法称为加载互锁(load interlock)
* 加载互锁和转发技术结合起来足以处理所有可能类型的数据冒险

#### 避免控制冒险
当处理器无法根据处于取指阶段的当前指令来确定下一条指令的地址时，就会出现控制冒险。
![pipe_jump_race](pictures/pipe_jump_race.png)
* 预测错误，0x016和0x020通过插入气泡被取消

### 异常处理
异常可以由程序执行从内部产生，也可以由某个外部信号从外部产生。其中内部异常有：
* halt指令
* 有非法指令和功能码组合的指令
* 取指或数据读写试图访问一个非法地址

在一个流水线化的系统中，异常处理需要解决以下的问题：
* 在一条流水线操作的周期内，有多条指令会引起异常，应报告正确的异常类型给操作系统
* 由于分支预测的错误，执行了本不该执行的异常代码
    * 如果一条指令在其处理中于某个阶段产生了一个异常，状态码stat被设置为异常的种类。异常状态和该指令的其他信息一起沿着流水线传播，直到它到达写回阶段
* 一条指令导致了一个异常，但它后面的执行在异常指令完成之前改变了部分状态
